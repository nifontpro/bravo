...
export const useDepartmentEdit = (setValue: UseFormSetValue<IDepartmentEditInput>) => {
	const {push, query} = useRouter()
	const departmentId = String(query.id)

	const {data: department, isLoading, isSuccess: isGetSuccess} = departmentApi.useGetByIdQuery(departmentId)
